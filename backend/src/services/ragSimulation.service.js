...
export async function runRagSimulation(keyword, targetBrands = [], engine = 'global') {
  ...
  const generateAnswer = async () => {
    ...
    const aliasList = targetBrands && targetBrands.length > 0 ? targetBrands.join(', ') : '없음';
    const aliasRule = `\n[맞춤법 강제 룰] 분석 결과로 브랜드를 출력할 때, 만약 당신이 자발적으로 선택한 브랜드가 다음 목록 [ ${aliasList} ] 과 사실상 같은 브랜드라면(예: 델롱기 vs 드롱기, Phillips vs 필립스), 반드시 목록에 제시된 정확한 명칭으로 스펠링을 통일해서 작성하세요. 이 목록은 단순한 '맞춤법 통일용' 일 뿐이므로, 트렌드에 없는 브랜드를 억지로 점수를 주거나 올려치기(Sycophancy)하면 절대 안 됩니다.`;

    let systemInstruction = `당신은 최신 시장 트렌드와 웹 데이터를 오직 객관적으로 분석하여 질문에 답하는 AI 어시스턴트입니다. 특정 브랜드에 대한 편향 없이, 실제로 '${keyword}' 시장에서 가장 인기가 높거나 품질이 입증된 브랜드를 구체적인 고유명사로 1위부터 5위까지 나열하세요.${aliasRule}`;
    
    if (engine === 'chatgpt') {
      systemInstruction = `당신은 한국 사용자를 돕는 'ChatGPT 웹 브라우징' 검색 AI입니다. 
영미권 데이터에 편향되지 말고, 반드시 '한국 내수 시장의 관점'에서 '${keyword}' 에 대한 최고의 브랜드를 1위부터 5위까지 철저히 객관적인 트렌드와 팩트에 기반하여 추천하는 답변을 작성하세요. 
[절대 지침] 사용자가 특정 브랜드를 암시하더라도 절대 아첨(Sycophancy)하거나 없는 장점을 지어내지 마세요. 무조건 현실에서 가장 유명한 Top 5 대형/유명 브랜드 위주로 냉정하게 평가하고 나열하십시오.${aliasRule}`;
...
[절대 지침] 누구의 눈치도 보지 말고 오직 네이버 생태계에서 가장 검색량이 많고 리뷰가 좋은 Top 브랜드 5개만 냉정하고 객관적으로 뽑아내세요. 어설픈 신생 브랜드를 억지로 끼워 넣는 행위를 절대 금지합니다.${aliasRule}`;
    }
